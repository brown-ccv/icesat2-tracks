...
for fi, i in zip(f_fft2[1:], np.arange(1, N_f + 1)):
    ...
    R = np.sum(
        theta[1] * np.cos(phi - THETA) * np.cos(phi - THETA)
        + theta[0] * np.sin(phi - THETA) * np.cos(phi - THETA)
    )
    I = np.sum(
        theta[1] * np.cos(phi - THETA) * np.sin(phi - THETA)
        + theta[0] * np.sin(phi - THETA) * np.sin(phi - THETA)
    )
...
ls = LombScargle(
    np.array(Gi["dist"]), np.array(Gi["heights_c_weighted_mean"]), fit_mean=False
)
...
plt.plot(
    t_base,
    np.array(Gi["heights_c_weighted_mean"])
    - np.array(Gi["heights_c_weighted_mean"]).mean(),
    c="black",
    label=" org. data",
)
...
m_single = theta_i[0] * np.sin(t_base_short * 2 * np.pi * fi) + theta_i[1] * np.cos(
    t_base_short * 2 * np.pi * fi
)
...
m_single3 = (
    -np.sign(sum(theta_i))
    * np.linalg.norm(theta_i)
    * np.sin(t_base_short * 2 * np.pi * fi + np.arctan2(theta_i[1], theta_i[0]))
)

z_i_fft = phih_from_data[abs(phih_from_data).argmax()]
m_single_fft = (
    -np.sign(z_i_fft.real + z_i_fft.imag)
    * (z_i_fft).real
    * np.sin(t_base_short * 2 * np.pi * fi + np.arctan2(z_i_fft.imag, z_i_fft.real))
)
...
def reconstruct_data_from_LS(LS, x_real_axis, freq):
    ...
    for fi in freq_seq:
        ...
        y_reconstruct += (
            theta[0]
            + theta[1] * np.sin(x_real_axis * 2 * np.pi * fi)
            + theta[2] * np.cos(x_real_axis * 2 * np.pi * fi)
        )
...
S_pwelch_fft = spec.wavenumber_spectrogram(
    x, np.array(Gi["heights_c_weighted_mean"]), L
)
...
S_pwelch_k, S_pwelch_dk = spec.calc_freq_LS(
    x,
    L,
    method="fftX2",
    minimum_frequency=None,
    maximum_frequency=None,
    samples_per_peak=0.5,
)

S_pwelch_ls = spec.wavenumber_spectrogram_LS_even(
    x, y, L, waven_method=S_pwelch_k, dy=None, ov=None, window=None, kjumps=2
)
...
for k in high_beams:
    ...
    # dd = np.gradient(dd)
    # np.isnan(dd).sum()
    S = spec.wavenumber_spectrogram_LS_even(
        np.array(x),
        np.array(dd),
        L,
        waven_method=S_pwelch_k,
        dy=None,
        ov=None,
        window=None,
        kjumps=2,
...
plt.pcolormesh(
    Gplot.x / 1e3, 1 / Gplot.k, Gplot, cmap=plt.cm.ocean_r, vmin=clev[0], vmax=clev[-1]
)
...
plt.plot(
    f_fft2_regular[1::1], (spec.LS_power_to_PSD(ls_power, y.size, df2)), "r", zorder=12
)
...
S_pwelch_k, S_pwelch_dk = spec.calc_freq_LS(
    ...
    Lpoints,
    method="LS_auto",
    minimum_frequency=f_fft2_regular[1],
    maximum_frequency=f_fft2_regular[-1],
    samples_per_peak=0.1,
)
...
for k in high_beams:
    ...
    # dd = np.gradient(dd)
    # np.isnan(dd).sum()
    S = spec.wavenumber_spectrogram_LS(
        np.array(Gi.dist),
        dd,
        Lmeters,
        waven_method=f_regular,
        dy=None,
        ov=None,
        window=None,
        kjumps=1,
...
plt.suptitle(
    "LS on uneven grid (dx= " + str(int(np.diff(Gplot.x).mean())) + " m)", y=0.95
)
gs = GridSpec(3, 3, wspace=0.1, hspace=0.7)  # figure=fig,
# clev=np.arange(0, 6, 0.1)*3
