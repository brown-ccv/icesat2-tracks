...
ls_auto_power_windowed = ls_windowed.power(
    fft_sub, normalization="psd", assume_regular_frequency=False
)

ls_gap = LombScargle(
    np.array(Gi["dist"])[~nan_mask], data_gap[~nan_mask], fit_mean=False
)
ls_gap_auto_power = ls_gap.power(
    fft_sub, normalization="psd", assume_regular_frequency=False
)

ls_gap_windowed = LombScargle(
    np.array(Gi["dist"])[~nan_mask], (data_gap * windows)[~nan_mask], fit_mean=False
)
ls_auto_power_gap_windowed = ls_windowed.power(
    fft_sub, normalization="psd", assume_regular_frequency=False
)

ls_gap_window1 = LombScargle(
    np.array(Gi["dist"])[~nan_mask],
    (data_gap * window_single)[~nan_mask],
    fit_mean=False,
)
ls_auto_power_gap_window1 = ls_windowed.power(
    fft_sub, normalization="psd", assume_regular_frequency=False
)
...
plt.plot(
    f_fft2[1::],
    spec.LS_power_to_PSD(ls_auto_power_gap_windowed, y.size, df2),
    label="LS_gap_windows",
)
plt.plot(
    f_fft2[1::],
    spec.LS_power_to_PSD(ls_auto_power_gap_window1, y.size, df2),
    label="LS_gap_1 window",
)
...
for fi in fft_sub:  # f_fft2[1:]:
    ...
    m1 += theta[0] * np.sin(t_base * 2 * np.pi * fi) + theta[1] * np.cos(
        t_base * 2 * np.pi * fi
    )

    theta = ls_windowed.model_parameters(fi)
    m1_windowed += theta[0] * np.sin(t_base * 2 * np.pi * fi) + theta[1] * np.cos(
        t_base * 2 * np.pi * fi
    )
    ...
    m1_gap += theta[0] * np.sin(t_base * 2 * np.pi * fi) + theta[1] * np.cos(
        t_base * 2 * np.pi * fi
    )

    theta = ls_gap_window1.model_parameters(fi)
    m1_gap_window1 += theta[0] * np.sin(t_base * 2 * np.pi * fi) + theta[1] * np.cos(
        t_base * 2 * np.pi * fi
    )

    theta = ls_gap_windowed.model_parameters(fi)
    m1_gap_windowed += theta[0] * np.sin(t_base * 2 * np.pi * fi) + theta[1] * np.cos(
        t_base * 2 * np.pi * fi
    )
...
spec_data_fft3 = 2.0 * (phih_from_data * phih_from_data.conj()).real / df3 / y.size**2
print("data fft sum:", df3 * spec_data_fft3.sum().data)

print("LS model var:", m1.var())
spec_LS_model_fft3 = 2.0 * (phih_from_LS * phih_from_LS.conj()).real / df3 / m1.size**2
...
spec_LS_model_gap_fft3 = (
    2.0 * (phih_from_LS_gap * phih_from_LS_gap.conj()).real / df3 / y.size**2
)
...
weights = gauss_weight(f_fft3, [0.001, 0.005, 0.045], [0, -0.5, -0.9], sigma_g=0.002)
...
plt.plot(
    f_fft3,
    M.runningmean(np.abs(phih_from_LS), 10, tailcopy=True),
    linewidth=1.2,
    c="blue",
    label="LS full data",
)
...
plt.plot(
    f_fft3,
    M.runningmean(np.abs(phih_from_LS_gap), 10, tailcopy=True),
    linewidth=1.2,
    c="red",
    label="LS gap data",
)
...
plt.plot(
    f_fft3, np.abs(phih_from_LS_gap) * weights, linewidth=0.5, c="orange", alpha=0.6
)
plt.plot(
    f_fft3,
    M.runningmean(np.abs(phih_from_LS_gap) * weights, 10, tailcopy=True),
    linewidth=1.2,
    c="orange",
    label="LS gap weighted",
)
...
def objective_func(
    params, data_x, Z_results, weight_func, nan_mask=None, plot_flag=False
):
    ...
    if plot_flag:
        ...
        chunk_l = 400
        chunk_iter = spec.create_chunk_boundaries(
        ...
        for chi in chunk_iter:
            ...
            plt.plot(
                ofsett + model_x(Z_results, weights * 0 + 1)[chi[0] : chi[-1]],
                linewidth=0.8,
                c="red",
            ...
            plt.plot(
                ofsett + model_x(Z_results, weights)[chi[0] : chi[-1]],
                linewidth=0.8,
                c="blue",
        ...
        v2 = np.round(
            (2.0 * (Z_results * Z_results.conj()).real / data_x.size**2).sum(), 4
        )
        plt.plot(
        ...
        v2 = np.round((2.0 * (Z2 * Z2.conj()).real / data_x.size**2).sum(), 4)
        plt.plot(
            abs(Z2), linewidth=0.8, c="blue", label="weighted Z(var: " + str(v2) + ")"
        )
        plt.plot(
        ...
        plt.ylim(
            np.percentile(abs(np.fft.rfft(data)), 1), abs(np.fft.rfft(data)).max() * 1.3
        )
...
f_list = [
    ...
    f_max,
    f_max,
]  # + 1 * (f_fft3[-1] - f_max)/2]#, f_max + 2 * (f_fft3[-1] - f_max)/3 ]
...
def gauss_weight_function(ff, params):
    ...
    if params is not None:
        ...
        weights = gauss_weight(
            f_fft3, [0.001, 0.005, 0.045], [0, -0.5, -0.9], sigma_g=params["sigma_g"]
        )
...
fitter = lmfit.minimize(
    objective_func,
    params,
    args=(data_gap, phih_from_LS_gap, constant_weight_function),
    kws={"nan_mask": nan_mask},
...
params.add("LF_amp", 1, min=0.2, max=1.5)
params.add("HF_amp", 1, min=0.2, max=1.5)
...
def objective_func(
    ...
    if plot_flag:
        ...
        plt.ylim(
            np.percentile(abs(np.fft.rfft(data)), 0.5),
            abs(np.fft.rfft(data)).max() * 1.3,
        )
...
fitter = lmfit.minimize(
    objective_func,
    params,
    args=(D_data, Z_data, tanh_weight_function),
    kws={"nan_mask": nan_mask},
...
plt.plot(
    M.runningmean(np.abs(phih_from_data), 20, tailcopy=True),
    linewidth=1.2,
    c="black",
    label="LS full data",
    zorder=12,
