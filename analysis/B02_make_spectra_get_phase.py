...
Gfilt = io.load_pandas_table_dict(
    track_name + "_B01_regridded", load_path
)  # rhis is the rar photon data
...
yy = np.copy(Gi["lons"])
yy[nan_mask] = np.interp(
    Gi["dist"][nan_mask], Gi["dist"][~nan_mask], yy[~nan_mask]
).shape
...
for k in all_beams:
    ...
    G = G.assign_coords(x_coord=("x", mapped_coords[:, 1])).assign_coords(
        y_coord=("x", mapped_coords[:, 2])
    )
    ...
    G = G.assign_coords(lon=("x", mapped_coords[:, 1])).assign_coords(
        lat=("x", mapped_coords[:, 2])
    )
    ...
    if plot_data_model:
        for i in np.arange(60, 120, 2):
            ...
            plt.plot(
                G.eta[0:-1] + xi_1,
                np.fft.irfft(G.sel(x=xi_1).Y_model_hat),
                "-",
                c=c1,
                linewidth=0.8,
                alpha=1,
            ...
            plt.plot(
                G.eta[0:-1] + xi_2,
                np.fft.irfft(G.sel(x=xi_2).Y_model_hat),
                "-",
                c=c2,
                linewidth=0.8,
                alpha=1,
            ...
            plt.text(
                xi_1 + G.eta[0].data,
                0.5,
                "  N=" + str(G.sel(x=xi_1).N_per_stancil.data),
            )
            plt.text(
                xi_2 + G.eta[0].data,
                0.5,
                "  N=" + str(G.sel(x=xi_2).N_per_stancil.data),
            )
...
plt.plot(
    Gplot.x / 1e3,
    G_LS_wmean.N_photons / 3 / Lmeters,
    c="black",
    label="ave Photons",
    linewidth=0.8,
)
