...
mask, k, weights, positions = define_wavenumber_weights_tot_var(
    GGk.mean("beam")["gFT_PSD_data"], m=3, variance_frac=0.33, verbose=True
)
...
def plot_instance(
    z_model, fargs, key, non_dim=False, title_str=None, fitter=None, view_scale=0.3
):
...
def plot_brute_force(fitter_brute):
    ...
    plt.plot(
        fitter_brute.brute_x0[1],
        fitter_brute.brute_x0[0],
        "+r",
        markersize=20,
        label="Brute force",
    )
...
# %% normalize data
key = "y_data"
amp_Z = G_x[key + "_normed"] = (GGx[key] - GGx[key].mean(["eta"])) / GGx[key].std(
    ["eta"]
)
...
def objective_func(pars, x, y, z, test_flag=False, prior=None):
    ...
    z_model = get_z_model(
        x, y, pars["K_prime"], pars["K_amp"], pars["alpha"], pars["group_phase"]
    )
...
params0.add(
    "alpha", 0, vary=True, min=alpha_grid[0], max=alpha_grid[-1], brute_step=d_alpha
)
params0.add(
    "group_phase",
    0,
...
dominant_dir_spread = Prior.loc[["pspr0", "pspr1", "pspr2", "pspr3", "pspr4", "pspr5"]][
    "mean"
].median()
...
for k_prime_max, Z_max in zip(k_list[::4], weight_list[::4]):
    ...
    F.ax3.axhline(
        prior_sel["alpha"][0] - prior_sel["alpha"][1], color="k", linewidth=0.7
    )
    F.ax3.axhline(
        prior_sel["alpha"][0] + prior_sel["alpha"][1], color="k", linewidth=0.7
    )
    plt.plot(
        fitter.params["group_phase"].value,
        fitter.params["alpha"].value,
        ".r",
        markersize=20,
    )
    ...
    cost_stack[k_prime_max] = xr.DataArray(
        fitter_brute.brute_Jout / N_data,
        dims=("angle", "phase"),
        coords={"angle": alpha_grid, "phase": phase_grid},
    )

    cost_stack[k_prime_max].coords["k"] = np.array(
        k_prime_max
...
for kindex in L.index:
    ...
    shift = int(
        -abs(cost_stack.phase - L["group_phase"][kindex]).argmin().data
        + cost_stack.phase.size / 2
    )
    # shift = int(- abs(cost_stack.phase - L['group_phase'][kindex]).argmin().data  )
    shift_list.append(shift)
    cost_stack_rolled[ii, :, :] = (
        cost_stack.sel(k=kindex).roll(phase=shift, roll_coords="phase").data
    )
...
gs = GridSpec(1, 5, wspace=0.25, hspace=0.2)  # figure=fig,
...
for g in group:
    plt.plot(
        GGk.sel(beam=g)["gFT_PSD_data"].data,
        GGk.k,
        "-k",
        color=col_dict[g],
        markersize=20,
...
plt.scatter(
    np.array(angle_list) * 180 / np.pi,
    np.array(k_list),
    s=(np.array(weight_list) * 4e1) ** 3,
    c=col.rascade1,
    label="min per freq.",
)
...
plt.plot(
    cost_wmean.angle * 180 / np.pi,
    k_list.mean() * cost_dd / cost_dd.mean(),
    c=col.rascade3,
    label="weighted mean BF",
)
...
ax1.axvline(
    best_guess * 180 / np.pi, color=col.blue, linewidth=1.5, label="best guess fitting"
)

ax1.axvline(
    (prior_sel["alpha"][0]) * 180 / np.pi, color="k", linewidth=1.5, label="prior"
)
ax1.axvline(
    (prior_sel["alpha"][0] - prior_sel["alpha"][1]) * 180 / np.pi,
    color="k",
    linewidth=0.7,
    label="prior uncertrainty",
)
ax1.axvline(
    (prior_sel["alpha"][0] + prior_sel["alpha"][1]) * 180 / np.pi,
    color="k",
...
plt.plot(
    data_normed.angle * 180 / np.pi,
    M.normalize(dn_unweighted.sel(phase=0, method="nearest")),
    label="mean",
)
plt.plot(
    cost_wmean.angle * 180 / np.pi,
    M.normalize(cost_wmean.sel(phase=0, method="nearest")),
    label="weighted mean",
)
...
GG["z_model"] = get_z_model(
    GG["dist"],
    GG["dist_y"],
    L.T["K_prime"],
    L.T["K_amp"] / amp_enhancement,
    L.T["alpha"],
    L.T["group_phase"],
)
...
for p in L2.T.items():
    ...
    params_big.add("K_amp_" + numi, ampi, vary=False, min=ampi * 0.5, max=ampi * 5)
...
for p in L2.T.items():
    ...
    grpi = p[1]["alpha"]
    params_big.add(
        "alpha_" + numi, grpi, vary=False, min=-0.95 * np.pi / 2, max=0.95 * np.pi / 2
    )
...
#
params_big.add(
    "angle_shift", 0, vary=False
)  # , min=-0.95 * np.pi /2, max=0.95 * np.pi /2)
...
def objective_func_big(params_big, x, y, z, test_flag=False):
    ...
    # print(alpha)
    z_model = get_z_model(
        GG["dist"], GG["dist_y"], K_prime_array, K_amp_array, alpha, group_phase_array
    )
    cost = (abs(z - z_model)) ** 2 / z.std() ** 2
...
GG["z_model"] = objective_func_big(
    params_big, GG["dist"], GG["dist_y"], GG["slopes"], test_flag=True
)
...
# %%
GG["z_model"] = objective_func_big(
    fitter.params, GG["dist"], GG["dist_y"], GG["slopes"], test_flag=True
)
plot_instance(GG, "slopes", "z_model", view_scale=0.6)

# %%
z_mdoel_init = objective_func_big(
    params_big, GG["dist"], GG["dist_y"], GG["slopes"], test_flag=True
)
objective_func_big(
    params_big, GG["dist"], GG["dist_y"], GG["slopes"], test_flag=False
).sum()
z_mdoel_fit = objective_func_big(
    fitter.params, GG["dist"], GG["dist_y"], GG["slopes"], test_flag=True
)
objective_func_big(
    fitter.params, GG["dist"], GG["dist_y"], GG["slopes"], test_flag=False
).sum()
