...
def recreate_fft_weights(GG_xi, k):
    ...
    data_fft = spec.Z_to_power(
        np.fft.rfft(y_gridded), np.diff(f_weight).mean(), GG_xi.eta.size
    )
...
for I in Gd.values():
    nan_fraction.append(
        np.sum(np.isnan(I["heights_c_std"])) / I["heights_c_std"].shape[0]
    )
...
plt.title(
    "x=" + str(np.round(GG_xi.x.data / 1e3, 1)) + "km, N=" + str(int(GGi.N_per_stancil))
)
...
if plot_data_model:
    for i in np.arange(30, 60, 2):
        ...
        plt.plot(
            eta + xi_1, FT.model(), "-", c="orange", linewidth=0.8, alpha=1, zorder=2
        )
        ...
        FT.b_hat = np.concatenate([GGi.gFT_cos_coeff, GGi.gFT_sin_coeff])
        plt.plot(
            eta + xi_2, FT.model(), "-", c="orange", linewidth=0.8, alpha=1, zorder=2
        )
        ...
        plt.text(
            ...
            + " N/2M= "
            + str(GG.sel(x=xi_1, method="nearest").N_per_stancil.data / 2 / kk.size),
        )
        plt.text(
            xi_2 + eta[0].data,
            ylims[-1],
            ...
            + " N/2M= "
            + str(GG.sel(x=xi_2, method="nearest").N_per_stancil.data / 2 / kk.size),
        )
...
mapped_coords = spec.sub_sample_coords(
    Gd[k]["dist"], x_coord_no_gaps, y_coord_no_gaps, S.stancil_iter, map_func=None
)
...
mapped_coords = spec.sub_sample_coords(
    Gd[k]["dist"], lons_no_gaps, lats_no_gaps, S.stancil_iter, map_func=None
)
...
plt.plot(
    G_gFT_power.k, np.nanmean(G_gFT_power, 1), "gray", label="mean gFT power data "
)
...
k_max_range = (
    Gmean.k[Gmean.mean("x").argmax().data].data * 0.75,
    Gmean.k[Gmean.mean("x").argmax().data].data * 1,
    Gmean.k[Gmean.mean("x").argmax().data].data * 1.25,
)
...
gs = GridSpec(3, 3, wspace=0.2, hspace=0.5)  # figure=fig,
# clev=np.arange(0, 6, 0.1)*3

# %matplotlib inline

clev = (
    M.clevels([Gmean.quantile(0.8).data * 1e4, Gmean.quantile(0.99).data * 1e4], 31)
    / 1e4
)
...
for pos, k, pflag in zip(
    ...
    Gplot = G_gFT_sel[
        k
    ].squeeze()  # .rolling(k=10, x=2, min_periods= 1, center=True).mean()
    # Gplot.mean('x').plot()
...
Gplot = (
    G_gFT_wmean.squeeze()
    .rolling(k=5, min_periods=1, center=True)
    .median()
...
plt.plot(
    Gplot.x / 1e3,
    G_gFT_wmean.N_photons / 3 / Lmeters,
    c="black",
    label="ave Photons",
    linewidth=0.8,
)
...
for k, I in G_rar_fft.items():
    ...
    plt.scatter(
        I.x / 1e3,
        I.sel(k=slice(k_max_range[0], k_max_range[2])).integrate("k") * 1e3,
        s=0.5,
        marker=".",
        c="blue",
        alpha=0.3,
...
plt.plot(
    Gplot.x / 1e3,
    Gplot.sel(k=slice(k_max_range[0], k_max_range[2])).integrate("k") * 1e3,
    ".",
    markersize=1.5,
    c="blue",
    label="FFT",
