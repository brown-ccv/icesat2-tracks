...
h_correction = (
    ALT07_cor["corrections"]["height_segment_mss"]
    + ALT07_cor["corrections"]["height_segment_ocean"]
    + ALT07_cor["corrections"]["height_segment_lpe"]
    + ALT07_cor["corrections"]["height_segment_dac"]
)
...
# subtract h_ref
mask = (ALT03c["delta_time"] >= ALT03["delta_time"].min()) & (
    ALT03c["delta_time"] <= ALT03["delta_time"].max()
)
ALT03c = ALT03c[mask]


ALT03["heights_c"] = ALT03["heights"] - np.interp(
    ALT03["delta_time"], ALT03c["delta_time"], ALT03c["dem_h"]
)
...
for ll in np.arange(latlims[0], latlims[1], dl):
    ...
    plt.plot(
        ALT07["ref"]["latitude"],
        ALT07["heights"]["height_segment_height"],
        "r.",
        markersize=0.8,
        alpha=1,
        label="ALT07 seg. heights",
    )
    ...
    # plt.legend()
    F.save_light(
        path=base_path + "plots/tests/",
        name="ALT03_ALT07_comparison_corrected" + str(ll),
    )
