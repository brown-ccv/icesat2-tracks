...
kk, ll, amps, mesh_shape = WaveEmulator.get_stancils_polar(
    k_abs, angle, size=200, dk=0.003, plot_flag=False, random=True
)
# kk, ll, amps, mesh_shape = WaveEmulator.get_stancils_polar(k_abs, angle, size=2, dk= 0.005 , plot_flag=False, random = True)

k_noise, l_noise, amp_noise, stancil_shape = WaveEmulator.get_stancils_polar(
    0.8, 0 * np.pi / 180, size=2, dk=1, mesh=True, plot_flag=False, random=True
)
...
G = np.vstack(
    [
        np.cos(np.outer(XX, k_all) + np.outer(YY, l_all)).T,
        np.sin(np.outer(XX, k_all) + np.outer(YY, l_all)).T,
    ]
).T
...
key_name = (
    ...
    + str(int(kk.size))
    + "_angle"
    + str(int(np.round(angle * 180 / np.pi, 0)))
)
...
k_list_gauss = np.arange(
    k_fft_max - 1.5 * k_sigma,
    k_fft_max + 1.5 * k_sigma + np.diff(k_fft).mean(),
    np.diff(k_fft).mean() / 2,
)
...
def plot_instance(
    GG3, data_key, model_key, non_dim=False, view_scale=0.3, fitter=None, title_str=None
):
    ...
    for beam, pos in zip(beam_list, [gs[0, :], gs[1, :]]):
        ...
        if non_dim:
            ...
            plt.plot(
                GGsel["dist"], GGsel[data_key], "-", c=next(col_list), linewidth=0.5
            )
...
for k_prime_max, Z_max in zip(k_list_interp, Z_list_interp):
    ...
    params.add("K_amp", amp_Z, vary=False, min=amp_Z * 0.5, max=amp_Z * 5)
    ...
    fitting_kargs = {"alpha_prior": None}
    F = plot_instance(
        GG, "slopes", "z_model", fitter=fitter_brute, view_scale=0.5, non_dim=True
    )
    ...
    cost_stack[k_prime_max] = xr.DataArray(
        ...
        dims=("angle", "phase"),
        coords={
            "angle": np.linspace(-np.pi / 2, np.pi / 2, N_grid),
            "phase": np.linspace(0, 2 * np.pi, N_grid),
        },
    )
...
angle_rad = np.concatenate(
    [-np.arctan2(A2["x_prime"], ymean)[::-1], np.arctan2(A2["x_prime"], ymean)]
)
cross_corr = np.concatenate(
    [
        autocrosscorr_func_1d(A1["slopes"][::-1], A2["slopes"][::-1])[::-1],
        autocrosscorr_func_1d(A1["slopes"], A2["slopes"]),
    ]
)
...
plt.text(
    np.mean(xlims), angle_true + 0.08, "True $\\alpha$=" + str(np.round(angle_true, 2))
)
...
plt.text(
    np.mean(xlims) / 1.5,
    angle_mean.data - 0.2,
    "mean $\\alpha$=" + str(np.round(angle_mean.data, 2)),
)
...
plt.plot(
    k_list_interp,
    k_list_interp * 0 + k_list_interp.mean(),
    ".r",
    markersize=10,
    zorder=12,
)
