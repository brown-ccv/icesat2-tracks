...
def make_slopes(
    G3, key="heights_c_weighted_mean", spreed=10, verbose=False, photon_min=5
):
...
stancil_iter = spec.create_chunk_boundaries_unit_lengths(
    Lmeters, xlims, ov=None, iter_flag=False
)
...
kk, ll, amps, mesh_shape = WaveEmulator.get_stancils_polar(
    k_abs, 0, size=1, dk=k_abs / 10, amp_std=k_abs / 20
)
...
def wavemodel(XX, YY, ks, ls, amps, group_phase=0, amp_height=1):
    ...
    b = (
        np.hstack([np.cos(group_phase) * amps, np.sin(group_phase) * amps]).squeeze()
        * amp_height
    )
    z_model = G @ b
...
for group_phase in np.linspace(0, 2 * np.pi, 4):
    ...
    kk, ll, amps, mesh_shape = WaveEmulator.get_stancils_polar(
        k_abs, akpha_rad, size=0, dk=dk, plot_flag=False
    )
...
def get_z_model(
    x_positions, y_position, K_prime, K_amp, alpha_rad, group_phase, dk, size=0
):

    K_abs = K_prime / np.cos(alpha_rad)
    kk, ll, amps, mesh_shape = WaveEmulator.get_stancils_polar(
        K_abs, alpha_rad, size=size, dk=dk, plot_flag=False
    )

    return wavemodel(
        x_positions, y_position, kk, ll, amps, group_phase=group_phase, amp_height=K_amp
    )
...
params.add(
    "K_prime", k_prime_abs, vary=False, min=k_prime_abs * 0.5, max=k_prime_abs * 1.5
)
...
fitter2 = LM.minimize(
    objective_func, params2, args=fitting_args, method="dual_annealing", max_nfev=None
)
# fitter3 = LM.minimize(objective_func, params2, args=fitting_args, method='emcee', nwalkers=100, steps=1500)
GG["z_model"] = objective_func(
    fitter2.params, GG["dist"], GG["dist_y"], GG["slopes"], test_flag=True
)
plot_instance(GG, "slopes", "z_model")
fitter2
...
# GG['slopes'] = zm
slopes1 = GG[GG["beam"] == "gt3r"]["slopes"]
GG["slopes"] = np.concatenate(
    [slopes1, np.roll(slopes1, int((0 * np.pi / 4) * (2 * np.pi / k_prime_abs) / dx))]
)
...
# fitting_kargs = {'size' :1}
fitter = LM.minimize(
    objective_func, params, args=fitting_args, method="dual_annealing", max_nfev=None
)
GG["z_model"] = objective_func(
    fitter.params, GG["dist"], GG["dist_y"], GG["slopes"], test_flag=True
)
...
# fitter3 = LM.minimize(objective_func, params2, args=fitting_args, method='emcee', nwalkers=100, steps=1500)
GG["z_model"] = objective_func(
    fitter2.params, GG["dist"], GG["dist_y"], GG["slopes_smth"], test_flag=True
)
