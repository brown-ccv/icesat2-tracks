...
def getATL07data(fileT, numpy=0, beam="gt1r", maxElev=1e6):
    ...
    along_track_distance = (
        ATL07[beam + "/sea_ice_segments/seg_dist_x"][:]
        - ATL07[beam + "/sea_ice_segments/seg_dist_x"][0]
    )
    ...
    elev_rms = ATL07[beam + "/sea_ice_segments/heights/height_segment_rms"][
        :
    ]  # RMS difference between modeled and observed photon height distribution
    seg_length = ATL07[beam + "/sea_ice_segments/heights/height_segment_length_seg"][
        :
    ]  # Along track length of segment
    height_confidence = ATL07[
        beam + "/sea_ice_segments/heights/height_segment_confidence"
    ][
        :
    ]  # Height segment confidence flag
    reflectance = ATL07[beam + "/sea_ice_segments/heights/height_segment_asr_calc"][
        :
    ]  # Apparent surface reflectance
    ssh_flag = ATL07[beam + "/sea_ice_segments/heights/height_segment_ssh_flag"][
        :
    ]  # Flag for potential leads, 0=sea ice, 1 = sea surface
    seg_type = ATL07[beam + "/sea_ice_segments/heights/height_segment_type"][
        :
    ]  # 0 = Cloud covered
    gauss_width = ATL07[beam + "/sea_ice_segments/heights/height_segment_w_gaussian"][
        :
...
plt.title("Freeboard")
plt.plot(
    T3["l"].get_group(0), T3["elev"].get_group(0) - mssh, "ko", markersize=1, alpha=0.5
)
...
plt.fill_between(
    xx, mean_freeboard - std_freeboard, mean_freeboard + std_freeboard, color="gray"
)
