...
histogram(phase_adjust, bins=[20], dim=["x"], weights=abs(co_spec)).rolling(
    **r_ave_kargs2
).mean().T.plot()
...
phase_hist.rolling(**r_ave_kargs2).mean().T.plot(
    cmap=plt.cm.ocean_r, levels=np.arange(-1, 18, 0.5)
)
...
plt.grid()
# %%
phase_hist.T.rolling(Y_model_hat_bin=5, x=10, center=True, min_periods=2).mean().plot(
    cmap=plt.cm.ocean_r, levels=np.arange(-1, 10, 0.5)
)
plt.ylabel("radients (1/pi)")
plt.grid()
...
phase.sel(x=slice(500000, 600000)).mean("x").rolling(
    k=20, center=True, min_periods=2
).mean().plot()  # .hist(bins= 40)
phase.sel(x=slice(500000, 600000)).mean("x").rolling(
    k=20, center=True, min_periods=2
).std().plot()  # .hist(bins= 40)
...
def get_coherency(d1, d2, Lint, dt=None):
    ...
    coh = signal.coherence(
        d1, d2, fs=1 / dt, window="hann", nperseg=Lint, noverlap=None, detrend="linear"
    )
