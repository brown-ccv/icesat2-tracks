...
data_weight = spec.Z_to_power(
    np.fft.rfft(data_filled), np.diff(f_weight).mean(), x.size
)
...
plt.plot(
    x[nan_mask], (b_hat * G_full).sum(1)[nan_mask], "-", color="orange", linewidth=0.5
)
# plt.xlim(1000,8000)
plt.ylim(-1, 1)
...
Z_power2 = (
    ((Z * (x.size / 2 + 1)) * (Z * (x.size / 2 + 1)).conj()).real
    * 2
    / (x.size * x_gap.size * dk)
)
Z_power2.sum() * dk
Z_power2.sum() * dk / (y[~nan_mask].var() - rr.var())
