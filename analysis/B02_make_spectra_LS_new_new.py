...
Gfilt = io.load_pandas_table_dict(
    track_name + "_B01_corrected", load_path
...
S_pwelch_k, S_pwelch_dk = spec.calc_freq_LS(
    ...
    samples_per_peak=0.005,
)
S_pwelch_k.shape
hkey = "heights_c_weighted_mean"

imp.reload(spicke_remover)
font_for_pres()
...
gs = GridSpec(3, 3, wspace=0.2, hspace=0.4)  # figure=fig,
...
Gplot = G_fft_wmean[
    :, G_fft_wmean.N_per_stancil >= G_fft_wmean.N_per_stancil.max().data * 0.9
]
plt.plot(
    Gplot.x / 1e3,
...
F.save_light(path=plot_path, name=track_name + "_exmpl_spec_LS_strong_L" + str(Lmeters))
