...
if __name__ == "__main__":
    ...
    track_name, batch_key, test_flag = (
        "20190219073735_08070210_004_01",
        "SH_batch02",
        False,
    )
...
if __name__ == "__main__":
    ...
    alpha = 35
    kk, ll = np.cos(alpha * np.pi / 180) * np.array([0.9 * k1, k1, 1.1 * k1]), np.sin(
        alpha * np.pi / 180
    ) * np.array([0.9 * k1, 1 * k1, 1.1 * k1])
...
if __name__ == "__main__":
    ...
    l_range = np.linspace(-0.1, 0.1, 60)
...
if __name__ == "__main__":
    ...
    stancil_k_mesh, stancil_l_mesh = k_mesh.reshape(k_mesh.size), l_mesh.reshape(
        l_mesh.size
    )
...
if __name__ == "__main__":
    ...
    for dk in np.arange(0.005, 0.03, 0.002):
        ...
        for size in [2, 5, 10, 30, 50, 100, 200]:
            ...
            k_list, l_list, amp_weights, stancil_shape = get_stancils_polar(
                k_abs,
                angle * np.pi / 180,
                size=size,
                dk=dk,
            ...
            k_noise, l_noise, amp_noise, stancil_shape = get_stancils_polar(
                0.8,
                0 * np.pi / 180,
                size=20,
                dk=0.3,
            ...
            b = (
                np.hstack([np.cos(phase) * amp_all, np.sin(phase) * amp_all]).squeeze()
                * amp
            )
            z_model = (G @ b).reshape(Ny, Nx)

            z_model.shape
            plt.contourf(x, y, z_model)
            plt.axis("equal")

            F.save_light(
                path=plot_path, name="fake_2d_dk" + str(dk) + "_s" + str(int(size))
            )
            plt.show()
