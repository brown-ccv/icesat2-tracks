...
ls_gap_win = LombScargle(
    np.array(Gi["dist"])[~nan_mask], (data_gap * win)[~nan_mask], fit_mean=False
)
ls_gap_win_power = ls_gap_win.power(
    f_fft2, normalization="psd", assume_regular_frequency=False
)
...
plt.plot(
    f_fft2, spec.LS_power_to_PSD(ls_gap_win_power, y.size, df2), label="LS_gap_win"
)
...
plt.plot(
    f_fft2[:-1], np.angle(np.fft.rfft(y * win)), c="k", label="windowed standard FFT"
)
...
for fi in f_fft2[1:]:  # f_fft2[1:]:
    ...
    y_model += theta[1] * np.cos(t_base * 2 * np.pi * fi) + theta[0] * np.sin(
        t_base * 2 * np.pi * fi
    )
...
spec_data_fft3 = 2.0 * (Y_from_data_win * Y_from_data_win.conj()).real / df3 / y.size**2
print("data * win fft sum:", df3 * spec_data_fft3.sum().data)

print("LS model var:", y_model.var())
spec_LS_model_fft3 = 2.0 * (Y_from_LS * Y_from_LS.conj()).real / df3 / y.size**2
...
def objective_func(
    params, data_x, Z_results, weight_func, freq, nan_mask=None, plot_flag=False
):
    ...
    if plot_flag:
        ...
        for chi in chunk_iter:
            ...
            plt.plot(
                ofsett + model_real_space(Z_results, weights * 0 + 1)[chi[0] : chi[-1]],
                linewidth=0.8,
                c="red",
...
fitter = lmfit.minimize(
    objective_func,
    params,
    args=(D_data, Z_data, tanh_weight_function, f_fft2),
...
plt.plot(
    M.runningmean(np.abs(Y_from_data_win), 20, tailcopy=True),
    linewidth=1.2,
    c="black",
    label="LS full data",
    zorder=12,
...
plt.xlim(0, 1000)  # old: plt.xlim(0, 10`00)
# %%
