...
dist_y_mean = np.sqrt(
    (B1["x_coord"] - B2["x_coord"]) ** 2 + (B1["y_coord"] - B2["y_coord"]) ** 2
).mean()
dist_y_mean
...
k_smooth = 5
some_angle = np.arctan2(
    GG0.Z_hat_imag.isel(dist_y=1).rolling(k=k_smooth).mean(),
    GG0.Z_hat_real.isel(dist_y=1).rolling(k=k_smooth).mean(),
)
...
GG0.sel(x=2e6, method="nearest").sel(k=slice(0.02, 0.05)).isel(
    dist_y=0
).Z_hat_imag.plot()
GG0.sel(x=2e6, method="nearest").sel(k=slice(0.02, 0.05)).isel(
    dist_y=1
...
# %%
k_smooth = 5
GG0.sel(x=2.05e6, method="nearest").rolling(k=k_smooth).mean().sel(
    k=slice(0.02, 0.06)
).isel(dist_y=0).gFT_PSD_model.plot()
GG0.sel(x=2.05e6, method="nearest").rolling(k=k_smooth).mean().sel(
    k=slice(0.02, 0.06)
).isel(dist_y=1).gFT_PSD_model.plot()

# %%

klim = 0, 0.06
Gi = (
    GG0.sel(x=2.05e6, method="nearest").isel(dist_y=0).sel(k=slice(klim[0], klim[1]))
)  # .rolling(k=10 ).mean()
Gi2 = (
    GG0.sel(x=2.05e6, method="nearest").isel(dist_y=0).sel(k=slice(klim[0], klim[1]))
)  # .rolling(k=5 ).mean()
...
plot_Z(
    Gi.gFT_cos_coeff.data * weight,
    Gi.gFT_sin_coeff.data * weight,
    k_wave=np.arange(Gi.k.size),
    line_c="k",
    head="o",
    head_size=60,
    shift=0,
...
plot_Z(
    Gi2.gFT_cos_coeff.data * weight,
    Gi2.gFT_sin_coeff.data * weight,
    k_wave=np.arange(Gi2.k.size),
    shift=0,
    line_c="r",
    head="+",
    head_size=120,
