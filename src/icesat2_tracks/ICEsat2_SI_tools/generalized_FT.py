...
def define_weights(stancil, prior, x, y, dx, k, max_nfev, plot_flag=False):
    ...
    if (type(prior[0]) is bool) and not prior[
        ...
        type(prior) is tuple
    ):  # prior= (PSD_from_GFT, weight_used in inversion), this is all other first iteration
...
class wavenumber_spectrogram_gFT(object):
    ...
    def cal_spectrogram(
        ...
        def calc_gFT_apply(stancil, prior):
            ...
            )  # Z_to_power_gFT(p_hat, dk, x.size,  Lpoints )
            ...
            if I is not np.nan:
                ...
                if x_pos.size > I["model_error_x"].size:
                    ...
                    I["model_error_x"] = I["model_error_x"][
                        0:-1
                    ]  # np.append(I['model_error_x'], I['model_error_x'][-1])
                    print("adjust y")
