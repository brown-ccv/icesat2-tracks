...
from icesat2_tracks.ICEsat2_SI_tools.spectral_estimates import (
...
try:
    ...
    MT.json_save(
        name="A01b_" + track_name + "_success",
        path=save_path,
        data={"reason": "ATL07 file not found, exit"},
    )
    exit()
...
def cut_rear_data(xx0, dd0, N_seg=20):
    ...
    while cut_flag:
        ...
        if (nsize / N_seg) < 1:
            break
        stencil_iter = create_chunk_boundaries(
            int(nsize / N_seg), nsize, ov=0, iter_flag=True
        )
...
def get_breakingpoints(xx, dd, Lmeter=3000):
    ...
    stencil_iter = spec.create_chunk_boundaries_unit_lengths(
        Lmeter, [xx.min(), xx.max()], ov=Lmeter * 3 / 4, iter_flag=True
    )
    iter_x = spec.create_chunk_boundaries_unit_lengths(
        Lmeter, [xx.min(), xx.max()], ov=Lmeter * 3 / 4, iter_flag=False
    )[1, :]
...
for k in beams:
    ...
    for TF, Tsel, TF_poleward in zip(
        ["TF1", "TF2"], [TF1, TF2], [TF1_poleward, TF2_poleward]
    ):
        ...
        if Tsel is None:
            ...
            if not TF_poleward:
                ...
                Tsel = Tsel.sort_values(
                    ("ref", "seg_dist_x"), ascending=False
                ).reset_index()

            # create local x axis
            Tsel["x"] = abs(
                Tsel["ref"]["seg_dist_x"] - Tsel["ref"]["seg_dist_x"].iloc[0]
            )
        ...
        if Tsel is None:
            ...
            data_density = Tsel.shape[0] / abs(
                Tsel["x"].max() - Tsel["x"].min()
            )  # datapoints per meters
        ...
        xx0, dd0 = np.array(Tsel["x"]), np.array(
            Tsel["heights"]["height_segment_height"]
        )
        if hemis is "SH":
        ...
        with threadpool_limits(limits=N_process, user_api="blas"):
            ...
            slope_test, pw_fit, breakpoint = get_breakingpoints(
                xx0[rear_mask], dd0[rear_mask], Lmeter=3000
            )
        ...
        if plot_flag:
            ...
            M.save_anyfig(
                plt.gcf(),
                name="A01b_" + track_name + "_" + k + "_" + TF,
                path=plot_path,
            )
            plt.close()
            # plt.show()
        ...
        DD_pos_start.loc[k, TF] = (
            Tsel.iloc[0]["ref"]["longitude"],
            Tsel.iloc[0]["ref"]["latitude"],
            Tsel.iloc[0]["ref"]["seg_dist_x"],
            Tsel.iloc[0]["time"]["delta_time"],
        )
        DD_pos_end.loc[k, TF] = (
            Tsel.iloc[-1]["ref"]["longitude"],
            Tsel.iloc[-1]["ref"]["latitude"],
            Tsel.iloc[-1]["ref"]["seg_dist_x"],
            Tsel.iloc[-1]["time"]["delta_time"],
        )
...
for Ti in DD_pos_start:
    ...
    for k in all_beams:
        if type(DD_pos_start[Ti][k]) is not tuple:
            ...
            print(
                "poleward check ",
                k,
                abs(DD_pos_start[Ti][k][1]) < abs(DD_pos_end[Ti][k][1]),
                abs(DD_pos_start[Ti][k][1]),
                abs(DD_pos_end[Ti][k][1]),
            )
    ...
    try:
        print(
            "poleward check sum",
            abs(TT_start[Ti][1]) < abs(TT_end[Ti][1]),
            abs(TT_start[Ti][1]),
            abs(TT_end[Ti][1]),
        )
    except:
...
for Ti, figp in zip(DD_pos_start, [121, 122]):
    ...
    for k in all_beams:
        if type(DD_pos_start[Ti][k]) is tuple:
            plt.scatter(
                np.deg2rad(DD_pos_start[Ti][k][0]),
                DD_pos_start[Ti][k][1],
                s=20,
                color="green",
            )  # , label='start')
        ...
        if (type(DD_pos_start[Ti][k]) is tuple) & (type(DD_pos_end[Ti][k]) is tuple):
            plt.plot(
                [np.deg2rad(DD_pos_start[Ti][k][0]), np.deg2rad(DD_pos_end[Ti][k][0])],
                [DD_pos_start[Ti][k][1], DD_pos_end[Ti][k][1]],
                color="black",
                linewidth=0.5,
            )  # , width=0.5, edgecolor='none', facecolor= 'black')
...
plt.legend()
M.save_anyfig(
    plt.gcf(), name="A01b_" + track_name + "_" + hemis + "_" + k, path=plot_path
)
...
if (DD_slope_mask.sum() > 1).sum() > 0:
    ...
    MT.json_save(
        ...
        data={
            ...
            "slope": DD_slope.where(pd.notnull(DD_slope), 0).to_dict(),
            "density": DD_data.where(pd.notnull(DD_data), 0).to_dict(),
        },
    )
    exit()
...
for TF, TF_poleward in zip(["TF1", "TF2"], [TF1_poleward, TF2_poleward]):
    ...
    if len(iregion) != 0:
        ...
        if type(iregion2) is str:  # This is the SH case
            ...
            org_files, remote_files, remote_names = (
                io.nsidc_icesat2_get_associated_file(ATL03_list, "ATL03")
            )
        ...
        DD["tracks"]["ATL03"] = [
            rem.split(".")[0] for rem in remote_names
        ]  # ['ATL03_'+i for i in ATL03_list]
        ATL03_proposed += org_files
        ATL03_remote_link += remote_files
        ATL03_remote += remote_names
        ...
        # add other pars:
        DD["pars"] = {
            "poleward": TF_poleward,
            "region": iregion2,
            "start": {
                "longitude": TT_start[TF][0],
                "latitude": TT_start[TF][1],
                "seg_dist_x": TT_start[TF][2],
                "delta_time": TT_start[TF][3],
            },
            "end": {
                "longitude": TT_end[TF][0],
                "latitude": TT_end[TF][1],
                "seg_dist_x": TT_end[TF][2],
                "delta_time": TT_end[TF][3],
            },
            "beams": list(DD_data[TF].index),
            "density": list(DD_data[TF]),
            "slope": list(DD_slope[TF]),
        }
...
product_directory, sd = (
    ATL03_remote_link[0].split("/")[4],
    ATL03_remote_link[0].split("/")[5],
)
