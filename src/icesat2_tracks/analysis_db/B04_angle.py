...
import imp
import copy
import icesat2_tracks.ICEsat2_SI_tools.spicke_remover
import datetime
...
plt.fill_between(kk, dir_interp_smth -spread_smth, dir_interp_smth +spread_smth, zorder= 1, color=color_schemes.green1, alpha = 0.2 )
...
def plot_instance(z_model, fargs , key, SM, non_dim=False, title_str= None , brute=False, optimze= False, sample= False,  view_scale = 0.3):
    ...
    import itertools
    col_list = itertools.cycle([color_schemes.cascade2, color_schemes.rascade2, color_schemes.cascade1, color_schemes.rascade1, color_schemes.cascade3, color_schemes.rascade3])
    ...
    for y_pos, pos in zip(beam_list, [ gs[0, :] , gs[1, :] ] ):
        ...
        plt.plot(x_concat[y_concat == y_pos], z_concat[y_concat == y_pos] ,  c=color_schemes.gray, linewidth = 1)
...
plt.fill_betweenx(Gk.k, (dir_interp_smth -spread_smth)* 180 /np.pi, (dir_interp_smth +spread_smth)* 180 /np.pi,  zorder= 1, color=color_schemes.green1, alpha = 0.2 )
plt.plot(dir_interp_smth * 180 /np.pi, Gk.k , '.', markersize = 1 , color=color_schemes.green1)
