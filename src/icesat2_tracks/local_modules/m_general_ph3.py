...
class figure_axis_xy:
    ...
    def save(self, name=None, path=None, verbose=True):
        ...
        name = (
            name
            if name is not None
            else datetime.date.today().strftime("%Y%m%d_%I%M%p")
        )
        # print(savepath)
        # print(name)
        extension = ".pdf"
...
class plot_polarspectra:
    ...
    def linear(self, radial_axis="period", circles=None, ax=None, fontsize=10):
        ...
        if self.data_type == "fraction":
            cm = (
                plt.cm.RdYlBu_r
            )  # brewer2mpl.get_map( 'RdYlBu','Diverging', 4, reverse=True).mpl_colormap
            colorax = ax.contourf(
                self.thetas, 1 / self.f, self.data, self.clevs, cmap=cm, zorder=1
            )  # ,cmap=cm)#, vmin=self.ctrs_min)
        elif self.data_type == "energy":
            cm = (
                plt.cm.Paired
            )  # brewer2mpl.get_map( 'Paired','Qualitative', 8).mpl_colormap
        ...
        if self.data_type == "fraction":
            ...
            cbar.set_label(
                "Energy Density (" + self.unit + ")", rotation=0
            )  # , fontsize=fontsize)
...
def set_timeaxis_days(ax, int1=1, int2=2, bymonthday=None):
    ...
    Day = dates.DayLocator(
        interval=int2, bymonthday=bymonthday
    )  # bymonthday=range(1,32)
    Day_dfmt = dates.DateFormatter("%d")
    Day2 = dates.DayLocator(
        interval=int1, bymonthday=bymonthday
    )  # bymonthday=range(1,32)
...
def boxmean(
    data, lon, lat, xlim, ylim
):  # , data_index, G.lon[:], G.lat[:], [160, -140+360], [-50 , -70]):
