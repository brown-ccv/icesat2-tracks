...
def cost(x, y):
    z = (
        4
        + np.sin(4 * 2 * np.pi * x / Lx)
        + np.sin(3 * np.pi * x / Lx - np.pi / 5)
        + np.cos(1 * 2 * np.pi * y / Ly)
        + np.sin(3 * np.pi * y / Ly - np.pi / 3)
    )
    return z**2
...
fitter = LM.minimize(
    objective_func, params, method="emcee", nwalkers=nwalkers, steps=100, pos=pos0.T
)
...
print(
    "print final diff:",
    cost_2d.min() - cost(fitter.params["x"].value, fitter.params["y"].value),
)
