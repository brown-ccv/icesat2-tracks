...
plt.plot(decay)
yy = (
    np.sin(k * xx * 2 * np.pi) * decay
    + np.sin(10 * k * xx * 2 * np.pi) * decay
    + 0.3 * np.random.randn(len(xx))
)
plt.plot(yy)
...
L = 400
imp.reload(specs)
S = specs.wavenumber_pwelch(
    yy, xx, L, ov=None, window=None, save_chunks=False, plot_chunks=False
)
