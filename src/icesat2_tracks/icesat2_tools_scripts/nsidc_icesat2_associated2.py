...
COMMAND LINE OPTIONS:
    ...
    -p X, --product: associated product to download
        ...
        ATL10: Sea Ice Freeboard - uses hemisphere ID here
...
def nsidc_icesat2_associated(
    ...
    remote_regex_pattern = (
        r"{0}-({1})_(\d{{4}})(\d{{2}})(\d{{2}})"
        r"(\d{{2}})(\d{{2}})(\d{{2}})_({2})({3})({4})_({5})_(\d{{2}})(.*?).{6}"
    )
    ...
    for input_file in file_list:
        ...
        SUB, PRD, HEM, YY, MM, DD, HH, MN, SS, TRK, CYC, GRN, RL, VRS, AUX = rx.findall(
            input_file
        ).pop()
        ...
        args = (PRODUCT, HEM, TRK, CYC, GRN, RL, regex_suffix)
...
def http_pull_file(
    remote_file, remote_mtime, local_file, TIMEOUT=None, RETRY=1, MODE=0o775
):
    ...
    retry_download(
        remote_file, LOCAL=local_file, TIMEOUT=TIMEOUT, RETRY=RETRY, CHUNK=CHUNK
    )
...
def retry_download(remote_file, LOCAL=None, TIMEOUT=None, RETRY=1, CHUNK=0):
    ...
    while retry_counter < RETRY:
        ...
        try:
            ...
            response = icesat2_toolkit.utilities.urllib2.urlopen(
