...
spec_power_gauss = np.max(spec_power) * np.exp(
    -(((f - f[spec_power.argmax()]) / f_std) ** 2)
)
...
weight = (
    (np.interp(kk_GFT, k, spec_power_gauss) / spec_power_gauss.max())
    * 0.8
    * instance2.var()
)
err = instance2 * 0 + 0.1
...
print(
    "data energy / mean model energy = ",
    np.array(dd_no_nans).var() / (GG.gFT_PSD_model.sum("k") * dk).mean("x").data,
)
