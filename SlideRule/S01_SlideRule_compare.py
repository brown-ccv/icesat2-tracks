...
# import s3fs
# %%
ID_name, batch_key, test_flag = io.init_from_input(
    sys.argv
...
B2_hdf5 = h5py.File(
    load_path_work + "B01_regrid" + "/" + ID_name + "_B01_regridded.h5", "r"
)
B3_hdf5 = h5py.File(
...
latR = [
    np.round(ID["pars"]["start"]["latitude"], 1),
    np.round(ID["pars"]["end"]["latitude"], 1),
]
lonR = [
    np.round(ID["pars"]["start"]["longitude"], 1),
    np.round(ID["pars"]["end"]["longitude"], 1),
]
latR.sort()
lonR.sort()
...
params_yapc = {
    ...
    "yapc": dict(
        knn=0, win_h=3, win_x=11, min_ph=4, score=50
...
for llen in [10, 20, 30]:
    ...
    ATL06_native[llen] = icesat2.atl06p(
        params, asset="nsidc-s3", resources=[track_name]
    )
    ATL06_yapc[llen] = icesat2.atl06p(
        params_yapc, asset="nsidc-s3", resources=[track_name]
...
def init_compare_figure(B2i, B3i, lims):
    ...
    axx[4] = F.fig.add_subplot(
        grid[1, -1], sharey=axx[2], yticklabels=[]
    )  # yticklabels=[])
...
for ll, ccol in zip([10, 20, 30], ["green", "darkorange", "red"]):
    ...
    add_data(
        ...
        10,
        ccol,
        "len=" + str(ll) + "| +" + str(np.round(delta_y, 2)),
        alpha=0.8,
        scatter=False,
    )
    # axx[2].plot(B3_atl06_native.geometry.y, -np.gradient(B3_atl06_native.h_mean)+0.02, 'darkgreen' )
...
params_yapc = {
    ...
    "t1": "2019-05-02T03:00:00",
    "yapc": dict(
        knn=0, win_h=6, win_x=20, min_ph=4, score=100
    ),  # use the YAPC photon classifier; these are the recommended parameters, but the results might be more specific with a smaller win_h value, or a higher score cutoff
    "poly": poly,
}
...
for wwin_x in [10, 20, 30]:
    ...
    ATL06_yapc_winx[wwin_x] = icesat2.atl06p(
        params_yapc, asset="nsidc-s3", resources=[track_name]
    )
...
for wwin_h in [0, 10, 20]:
    ...
    # Run the parallel version of the ATL06 algorithm, for a specified granule:
    # case_name = str(wwin_x) +'_' +str(wwin_h)
    ATL06_yapc_winh[wwin_h] = icesat2.atl06p(
        params_yapc, asset="nsidc-s3", resources=[track_name]
    )
...
for ll, ccol in zip(list(ATL06_yapc_winx.keys()), ["green", "darkorange", "red"]):
    ...
    add_data(
        ...
        10,
        ccol,
        "win_x=" + str(ll) + "| +" + str(np.round(delta_y, 2)),
        alpha=0.8,
        scatter=False,
    )
    # axx[2].plot(B3_atl06_native.geometry.y, -np.gradient(B3_atl06_native.h_mean)+0.02, 'darkgreen' )
...
for ll, ccol in zip(list(ATL06_yapc_winh.keys()), ["green", "darkorange", "red"]):
    ...
    add_data(
        ...
        10,
        ccol,
        "win_h=" + str(ll) + "| +" + str(np.round(delta_y, 2)),
        alpha=0.8,
        scatter=False,
    )
    # axx[2].plot(B3_atl06_native.geometry.y, -np.gradient(B3_atl06_native.h_mean)+0.02, 'darkgreen' )
...
hm = ax.scatter(
    gdfi[mask].geometry.y,
    gdfi[mask].height,
    c=gdfi[mask]["yapc_score"],
    s=2,
    cmap="viridis",
    vmin=0,
    vmax=256,
